...
from(
	...
	from(		 
		...
		where username in ('960120', 'JiH', 'cai', 'ccccc', 'ilovechicken','en', 'madame_id', 'ncjc','nielie','raz24','student335') and course_editions.edition_id = enrollments.edition_id
			...
			and course_editions.course_id not in (
								select course_editions.course_id
...
/* By course evaluations for existing user */
...
/* By promoting interests for existing user */
...
from(
	...
	where courses.course_id = topic_interests.course_id and username in ('960120', 'JiH', 'cai', 'ccccc', 'ilovechicken','en', 'madame_id', 'ncjc','nielie','raz24','student335') 
		and topic_id in (5, 82, 72, 52) and courses.course_id not in (select course_editions.course_id
														  from enrollments, course_editions, courses
														  where enrollments.edition_id = course_editions.edition_id
															  and course_editions.course_id = courses.course_id
															  and username = 'ww')
...
/* By improving skills for existing user */
...
from(
	...
	where courses.course_id = skill_rankings.course_id and username in ('960120', 'JiH', 'cai', 'ccccc', 'ilovechicken','en', 'madame_id', 'ncjc','nielie','raz24','student335') 
		and skill_id in (57, 7, 40, 35, 77) and courses.course_id not in (select course_editions.course_id
																		  from enrollments, course_editions, courses
																		  where enrollments.edition_id = course_editions.edition_id
																			and course_editions.course_id = courses.course_id
																			and username = 'ww')
